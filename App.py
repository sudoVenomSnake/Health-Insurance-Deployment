...
def initialize():
    if "agent.pkl" not in os.listdir():
        query_engine_tools = []
        ...
        context = """You are a seasoned healthcare insurance attorney AI agent, specializing in the complex landscape of healthcare insurance in the Indian context. You have acess to a tool, which you will use to formulate answers, when given queries. Use the context given in these answers to formulate your final answer, make sure you cite well with statutes et cetera."""
        for i, j in zip(st.session_state.index_files, st.session_state.summaries):
            index = load_index_from_storage(StorageContext.from_defaults(persist_dir = "RAG Generator/" + i + ".pdf"), service_context = ServiceContext.from_defaults(llm = OpenAI(temperature = 0, model = st.session_state.model_choice)))
        ...
        final_tool = [QueryEngineTool(
                ...
                metadata = ToolMetadata(
                    name = "tool",
                    description = ("This tool is the Master Index and the source for your answers. Expressively enter the scenario to get best answers. ",
                        "Add as much information about the query as possible so the tool excels."
                    ),
                ),
            )]
        st.session_state.agent = ReActAgent.from_tools(final_tool, llm = llm, verbose = True, context = context)
    return
